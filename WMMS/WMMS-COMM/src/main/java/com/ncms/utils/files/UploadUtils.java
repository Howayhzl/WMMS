...
public class UploadUtils {
	 ...
	 * PropertiesLoader("properties/sysConfig.properties"); private String
	 * basePath = loader.getProperty("UploadUrls");
	 */
	...
	public UploadUtils() {
		...
		extMap.put("files",
				"doc,docx,xls,xlsx,ppt,pdf,htm,html,txt,zip,rar,gz,bz2,gif,jpg,jpeg,png,bmp");
	}
		...
		if (fiList != null) {
			for (FileItem item : fiList) {
				...
				if (StrUtil.isBlank(newName)) {// 防止文件名重名。
					String fileNameWithoutExtension = FileUtils
							.getFileNameWithoutExtension(name);// 没有扩展名的文件名
					newName = fileNameWithoutExtension + "_"
							+ System.currentTimeMillis();// +"."+fileExtension
				}
				String fileExtension = FileUtils.getFileExtension(name);// 取扩展名
				fileName = StrUtil.isNotBlank(newName) ? newName.replace(" ",
						"") + "." + fileExtension : name.replace(" ", "");
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Map<String, Object> newuploadFile(String imageFile, String newName,
			String path, String imageName) {
		/*
		 * BASE64Decoder decoder = new BASE64Decoder(); // Base64解码 byte[]
		 * imageByte = null; try { imageByte = decoder.decodeBuffer(imageFile);
		 * for (int i = 0; i < imageByte.length; ++i) { if (imageByte[i] < 0)
		 * {// 调整异常数据 imageByte[i] += 256; } } } catch (Exception e) {
		 * e.printStackTrace(); }
		 */
		...
		//
		// 上传 datacollecttypeId
		// List<FileItem> fiList = (List<FileItem>)
		// fieldsMap.get(UploadUtils.FILE_FIELDS);
		// if (fiList != null) {
		// for (FileItem item : fiList) {
		...
		if (StrUtil.isBlank(newName)) {// 防止文件名重名。
			String fileNameWithoutExtension = FileUtils
					.getFileNameWithoutExtension(imageName);// 没有扩展名的文件名
			newName = fileNameWithoutExtension + "_"
					+ System.currentTimeMillis();// +"."+fileExtension
		}
		String fileExtension = FileUtils.getFileExtension(imageName);// 取扩展名
		fileName = StrUtil.isNotBlank(newName) ? newName.replace(" ", "") + "."
				+ fileExtension : imageName.replace(" ", "");
		fileName = fileName.replace(" ", "");// 去掉文件名中的空格
		newName = newName.replace(" ", "");
		...
		/*
		 * try { // 生成文件 File imageFile = new File(filename);
		 * imageFile.createNewFile(); if(!imageFile.exists()){
		 * imageFile.createNewFile(); } OutputStream imageStream = new
		 * FileOutputStream(imageFile); imageStream.write(bt);
		 * imageStream.flush(); imageStream.close(); } catch (Exception e) {
		 * e.printStackTrace(); }
		 */
	...
	public JSONObject patchUploadFile(String imageFile, String newName,
			String path, String imageName) {
	...
	private String validatePathFields(String path) {
		...
		if (!uploadDir.isDirectory()) {// 检查目录
			...
			/*
			 * // 获取上传临时路径 tempPath = tempPath + "/"; File file = new
			 * File(tempPath); if (!file.exists()) { file.mkdirs(); }
			 */
		}
	...
	*//**
	...
	private String saveNewFile(byte[] bt, String newName) {
		...
		if (bt.length > maxSize) { // 检查文件大小
			...
			// String bin_path =
			// savePath.substring(savePath.lastIndexOf("\\")+1,savePath.length());
