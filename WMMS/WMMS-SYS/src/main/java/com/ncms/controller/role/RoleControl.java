...
public class RoleControl{
	...
	public BackEntity querySysRole(String roleCode,String roleName,String roleNote,int roleState,
		...
		Page<Map<String,Object>> SysRolePage = sysRoleVOService.querySysRoleByName(map,cur_page_num,page_count);
	...
	public boolean exist(String roleCode){
		...
		List<SysRole> list = sysRoleService.findByEntity(role);
	    ...
	    */
		@RequestMapping(value="/role/delete", method = RequestMethod.POST)
		 public BackEntity deleteRole(@RequestBody List<String> items) {
		    	String res = sysRoleVOService.deleteRole(items);
	    ...
	    public BackEntity openUseRole(@RequestBody List<String> items) {
	    	String res = sysRoleVOService.openUse(items);
	    	if(res.equals(RESULT.SUCCESS_1)){
	    ...
	    public BackEntity closeUseRole(@RequestBody List<String> items) {
	    	String res = sysRoleVOService.closeUse(items);
	    	if(res.equals(RESULT.SUCCESS_1)){
		...
		public BackEntity queryRoleById(@PathVariable("roleId") String roleId){
			SysRole sysRole = sysRoleVOService.queryRoleById(roleId);
	     ...
	     */
	 	...
	 	public BackEntity insertRole(SysRole sysRole) {
	 		...
	 		String res = sysRoleVOService.insertRole(sysRole);
	 		if(res.equals(RESULT.SUCCESS_1)){
		...
		public BackEntity updateRole(SysRole sysRole){
	    	String res =sysRoleVOService.updateRole(sysRole);
	    	if(res.equals(RESULT.SUCCESS_1)){
		...
		public BackEntity queryAllUser(String roleId){
	    	Map<String,Object> resMap =sysRoleVOService.queryAllUser(roleId);
		...
		public BackEntity queryLeftUser(String roleId){
			List<Map<String, Object>> resMap =sysRoleVOService.queryUsedUser(roleId);
	    	return BackEntity.ok("查询用户成功！",resMap);
		}
		/**
	     * 未关联用户
		...
		public BackEntity queryRightUser(String roleId){
			List<Map<String, Object>> resMap =sysRoleVOService.queryUnusedUser(roleId);
	    	return BackEntity.ok("查询用户成功！",resMap);
		}
		/**
	     * 新建用户与角色关联关系
		...
		public BackEntity queryMenuTree(){
			List<Map<String, Object>> resMap =sysRoleVOService.queryMenuTree();
		...
		public BackEntity queryMenuIdByRoleId(@PathVariable("id") String roleId){
			List<String> resMap =sysRoleVOService.queryMenuIdByRoleId(roleId);
